...
{
    std::string concat;
    concat.reserve(iv.size() + public_value.size() + ciphertext.size());

    concat.append(reinterpret_cast<const char *>(iv.data()), iv.size());
    concat.append(reinterpret_cast<const char *>(public_value.data()), public_value.size());
    concat.append(ciphertext);
